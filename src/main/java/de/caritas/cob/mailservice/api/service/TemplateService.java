...
public class TemplateService {
  ...
  @NonNull private final TranslationService translationService;
   ...
   */
  public Optional<String> render(TemplateDescription desc, MailDTO mail, Map<String, Object> data)
    ...
    data.putAll(
        getTranslationMapAndDefaultToGermanIfNotFound(mail.getLanguage(), mail.getDialect()));
    ...
    return translationsArePresentAndNotEmpty(mail)
        ? getProcessedHtml(data, mail.getLanguage(), templateFilename, mail.getDialect())
  ...
  private boolean translationsArePresentAndNotEmpty(MailDTO mailDTO) {
    var translations =
        translationService.tryFetchTranslations(
            mailDTO.getLanguage().getValue(), mailDTO.getDialect());
    return translations.isPresent() && !translations.get().isEmpty();
  }

  private Map<String, String> getTranslationMapAndDefaultToGermanIfNotFound(
      LanguageCode language, Dialect dialect) {
    return translationService
        .tryFetchTranslations(language.getValue(), dialect)
        .orElse(
            translationService
                .tryFetchTranslations(LanguageCode.DE.getValue(), dialect)
                .orElse(new HashMap<>()));
  }
  ...
  public String getRenderedSubject(
      TemplateDescription templateDescription, Map<String, Object> templateData, MailDTO mailDTO) {
    ...
    if (subjectKey == null) {
      log.warn(
          "Subject key is null for template {}",
          templateDescription.getHtmlTemplateFilename().get(LanguageCode.DE));
  ...
  private String getRenderedSubjectForSubjectKeyNotNull(
      MailDTO mailDTO, StringSubstitutor stringSubstitutor, String subjectKey) {
    var translationKeys =
        getTranslationMapAndDefaultToGermanIfNotFound(mailDTO.getLanguage(), mailDTO.getDialect());
   ...
   */
  private List<String> getMissingTemplateFields(
      TemplateDescription mailTemplate, Map<String, Object> templateData) {
